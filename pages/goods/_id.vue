	...
	<div class="main w1210 mt10 bc">
		...
		<div class="breadcrumb">
			<h2>当前位置：
				...
				{{goodsInfo.goods_name}} - {{goodsInfo.spec_info.id_txt}}</h2>
		</div>
		<!-- 面包屑导航 end -->
		...
		<div class="goods_content fl mt10 ml10">
			...
			<div class="summary">
				...
				<div class="preview fl">
					...
					分配到模板的时候，把第一幅图片分配到 上面的midpic 中，
					其中大图分配到 a 标签的href属性，
					中图分配到 img 的src上。 
					下面的smallpic 则表示小图区域，格式固定，
					在 a 标签的 rel属性中，分别指定了中图（smallimage）和大图（largeimage），
					img标签则显示小图，按此格式循环生成即可，
						...
						<div class="smallpic_wrap">
							<ul>
								...
								<li v-for="(v,k) in goodsInfo.photos" :key = "k">
									...
									:rel="'{gallery: \'gal1\', smallimage: \''+v.bigimg+'\',largeimage: \''+v.xbigimg+'\'}'">
									<img :src="v.smimg"></a>
								</li>
							</ul>
						</div>
				...
				<div class="goodsinfo fl ml10">
					<ul>
						...
						<li class="star"><span>商品评分：</span> <strong></strong><a href="">(已有{{goodsInfo.comment_count}}人评价)</a></li> <!-- 此处的星级切换css即可 默认为5星 star4 表示4星 star3 表示3星 star2表示2星 star1表示1星 -->
					</ul>
					<form action="" method="post" class="choose">
						<ul>
							<li v-for="(v,k) in goodsInfo.spec_list" :key="k" class="product">
								<dl>
									<dt>{{v.spec_name}}：</dt>
									<dd>
										<a @click="selectSpec(v.id,vop.id)" v-for="(vop,kop) in v.options" :key="kop" :class="{selected:goodsInfo.spec_info.id_list.indexOf(v.id+':'+vop.id)!=-1}" href="javascript:;">{{vop.option_name}}</a>
							...
							<li>
								<dl>
									...
									<dd>
										...
										<input type="text" name="amount" v-model="buyCount" class="amount" @blur="update_num()"/>
						...
						<div class="comment_summary">
							...
							<div class="percent fl">
								...
								<dl>
									...
									<dd><div v-bind:style="{width:comments.ratio.bad+'px'}"></div></dd>
								</dl>
							</div>
							<div class="buyer fl">
								<dl>
...
export default {
    ...
    async mounted() {
		...
		// 商品评论
		let {data:comment_data} = await this.$axios.$get("/web-service/comments/" + this.goodsInfo.spuid)
    ...
    methods: {
		...
		update_num() {
			// 正则表达式判断
			let rule = /^\d+$/
			let falg = rule.test(this.buyCount)
			if(false) {
				if(this.buyCount < 1){
					this.buyCount = 1
				}
        ...
        selectSpec(specId,optionId) {
            ...
            // spec_info将我们新选中的数值替换老的sku组合信息
            let reg = new RegExp(specId + ":\\d+")
            // 第一个参数 正则表达式 
            // 第二个参数 新的数值
			let idLists = this.goodsInfo.spec_info.id_list.split('|');

            //let newId = this.goodsInfo.spec_info.id_list.replace(reg,specId + ':' + optionId)
            let newId = "";
			for(var i = 0; i < idLists.length; i++){
				var id = idLists[i];
				if(newId != "")
					newId += '|';
				if(id.split(':')[0] == specId)
					newId +=specId + ':' + optionId;
				else
					newId +=id;
			}
